...
project_name = 'FOD_avg'
project_path =  f'FL/{project_name}'
server_project_name = f"{project_name}_server"

server_energy = EnergyMonitoring(project_path, server_project_name, parameters, energy_calc_frequency, pid, mode='FL')
...
def client_fn(context: Context):
    ...
    client_yaml_path = f"./client_data.yaml" 
    yolo_yaml_path = f"./yolov11n_cus.yaml"

    # model = YOLO("yolov11_base.pt").to(device)
    model = YOLO(yolo_yaml_path, task='detect')
    model_name_pt = f"yolo_base.pt"
    model.load(model_name_pt)
...
def simulate():
    ...
    backend_config = {"client_resources": {"num_cpus": 1, "num_gpus": 1.0}} if device.type == "cuda" else {"client_resources": {"num_cpus": 1, "num_gpus": 0.0}}
...
def main():
    ...
    save_model = f'yolo_base.pt'
    model.save(save_model)
    # torch.save(model.state_dict(),'yolov11_base.pth')

    start_time = time.time()
    simulate()
    end_time = time.time()
    time_elapsed = end_time - start_time
    print(f"Total time taken for completion : {time_elapsed}")
    # Run the summarization function
    # summarize_energy_data()
    server_energy.final_calculation()
    # server_energy.aggregate_communication_energy()
